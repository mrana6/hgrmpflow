...
from differentiable_rmpflow.learning.controllers import ScalarMetric, MetricCholNet, ComposedPotentialNet, Exp
...
leaf_metric_fn = IdentityMetric()
n_hidden = 100
scaling_fn = nn.Sequential(
			nn.Linear(n_dims, n_hidden), nn.ReLU(),
			nn.Linear(n_hidden, n_hidden), nn.ReLU(),
			nn.Linear(n_hidden, 1), Exp(eps=torch.tensor(1e-12)))
leaf_pot_fn = ComposedPotentialNet(n_inputs = n_dims,
								   latent_taskmap=latent_taskmap,
								   latent_potential=latent_pot_fn,
								   goal=torch.zeros(1,2),
								   output_scaling_fn=scaling_fn,
								   normalization_scaling=scaling,
								   normalization_bias=translation)

leaf_rmp = NaturalGradientDescentMomentumController(G=leaf_metric_fn, Phi=leaf_pot_fn)
learner_model = leaf_rmp
learner_model.return_natural = False
learner_model.train()
