...
t0 = time.time()
traj = generate_trajectories(root, x_init=x0, t_init=0., t_final=T, t_step=dt, order=order, method='rk4', return_label=False)
tf = time.time()
...
plot_traj_2D(traj_np, '--', 'b', order=2)
